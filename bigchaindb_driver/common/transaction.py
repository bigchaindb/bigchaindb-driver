...
class Transaction(object):
    ...
    def transfer(cls, inputs, recipients, asset_id, metadata=None):
        """A simple way to generate a `TRANSFER` transaction.
            ...
            Returns:
                :class:`~bigchaindb.common.transaction.Transaction`
        """
        (inputs, outputs) = cls.validate_transfer(inputs, recipients, asset_id,
                                                  metadata)
    ...
    def from_dict(cls, tx, skip_schema_validation=True):
        """Transforms a Python dictionary to a Transaction object.
            ...
            Returns:
                :class:`~bigchaindb.common.transaction.Transaction`
        """
        operation = tx.get('operation', Transaction.CREATE) \
            if isinstance(tx, dict) else Transaction.CREATE
    ...
    def resolve_class(operation):
        """For the given `tx` based on the `operation` key
        return its implementation class
        """
